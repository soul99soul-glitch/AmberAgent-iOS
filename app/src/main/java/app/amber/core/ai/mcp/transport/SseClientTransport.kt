package app.amber.core.ai.mcp.transport
    ...
    override suspend fun send(message: JSONRPCMessage, options: TransportSendOptions?) {
        ...
        try {
            ...
            if (!response.status.isSuccess()) {
                error("Error POSTing to MCP endpoint (HTTP ${response.status})")
            }
    ...
    private suspend fun CoroutineScope.collectMessages() {
        try {
            session.incoming.collect { event ->
                ...
                when (event.event) {
                    "error" -> {
                        val error = IllegalStateException("MCP server sent an SSE error event")
                        _onError(error)
                        throw error
                    }
