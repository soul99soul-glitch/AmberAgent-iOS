...
| Phase 2 | 源码与模拟器验证完成 | T1 记忆 Score 与注入 Noul 同请求并提前启动；T2 输出投影按会话/调用/内容/策略固定并重放；T3 spawn 一次请求合并模型、角色和对齐题，followup 仅模型题；全 shadow 的 T3 在子任务启动后异步观测，不阻塞本地选择。 |
| Phase 3 | 源码与模拟器验证完成 | 工具发现增加用户原话和合适工具题；记忆筛查阈值 0.8；上下文块合并及结构化强保留；模型事实及单独数据范围；审批优先静态事实；网页候选/state 对齐、handback 与工具说明更新。 |
| Phase 4 | UI 与模拟器视觉验证完成 | 推荐配置需确认，五用途设为 shadow；用量区展示逐用途差异、等待、回退、工具使用、隐藏后重读和子任务终态；当前会话的 composer 用量面板展示本轮 Jev 摘要。缺数据的指标显示“暂无数据”。 |
...
- KMP：`:ai-core:jvmTest` 118 项、`:feature:tools:api:jvmTest` 29 项、`:shared:jvmTest` 38 项通过，失败/错误/跳过均为 0；`:shared:linkDebugFrameworkIosSimulatorArm64` 通过，Swift 头文件导出审批静态事实与纯搜索预览接口。
- iOS 模拟器：基础回归 `/private/tmp/jev-upgrade-focused-6.xcresult` 实际执行 453 项，453 通过；新增指标与受影响后台/聊天回归 `/private/tmp/jev-upgrade-metrics-3.xcresult` 实际执行 320 项，320 通过，两次均 0 失败、0 跳过。目标为 iPhone 17 Pro、iOS 26.5、arm64。已目视检查设置页默认、320pt AX3、完整指标长页与 composer 摘要截图，未见重叠或截断。
...
- 数字指标覆盖首个可见流式 chunk、模型步骤、缓存命中比例、T1/T2/T3 等待与 late/drop、工具 top1 差异及曝光数比、记忆 ID 重合/筛查命中、上下文隐藏占比与同参重读、模型/角色反事实差异、子任务权威终态，以及网页 handback/完成率。“下一步是否使用新暴露工具”只对前台 `tool_search` 有模型轮次观察点；后台与 Recipe discovery 不进入该比率分母，设置页明确标为前台口径。
